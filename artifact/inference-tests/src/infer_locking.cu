...
extern bool infer_start;
extern uint64_t num_call;
...
InferEngine::inferGraph(bool record_stats) {
    ...
    auto start = Clock::now();
    auto err = cudaGraphLaunch(cudaGraphInstance, stream);
    infer_start = false;
    if (err != cudaSuccess) {
        std::cout << "ERROR: TensorRT inference failed" << std::endl;
        return false;
...
main(int argc, char* argv[])
{
    ...
    int num_infer = 10;
    std::cout << "Warmup complete. Starting inference. (10 times)" << std::endl;
    for (int i = 0; i < num_infer; i++) {
        if (useGraph) {
            engine->inferGraph(true);
        } else {
            engine->infer(true);
        }
    }
    ...
    engine->stats();
    std::cout << "locking involved in invoking DL model: " << (num_call / num_infer) << " locking operations" << std::endl;
