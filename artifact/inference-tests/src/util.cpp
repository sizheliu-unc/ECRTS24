...
float* RGBImageReader::process()
{
    ...
    if (mPPM.h == H && mPPM.w == W)
    {
        for (int c = 0; c < C; c++)
        {
            ...
            {
                buffer[c * HW + j] = (static_cast<float>(mPPM.buffer[j * C + c])/mPPM.max - mMean[c]) / mStd[c];
            }
        }
    }
    else
    {
        std::cout << "Required height: " << H << "actual height: " << mPPM.h << "Required width: " << W << "actual width: " << mPPM.w ;
